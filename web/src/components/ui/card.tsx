...
const cardVariants = cva(
  "group/card flex flex-col gap-(--card-spacing) overflow-hidden rounded-3xl py-(--card-spacing) text-sm [--card-spacing:--spacing(4)] has-data-[slot=card-footer]:pb-0 has-[>img:first-child]:pt-0 data-[size=sm]:[--card-spacing:--spacing(3)] data-[size=sm]:has-data-[slot=card-footer]:pb-0 *:[img:first-child]:rounded-t-3xl *:[img:last-child]:rounded-b-3xl",
  {
    variants: {
      variant: {
        // Warm cream fill, no border — this is the card used on nearly every
        // page in the approved design.
        cream: "bg-secondary text-secondary-foreground",
        // Plain white card with a subtle ring — used where the surface needs
        // to read as "elevated" against a cream page background (auth card,
        // landing pricing card).
        default: "bg-card text-card-foreground ring-1 ring-foreground/10",
        // Transparent + dashed border, centered empty-state content (e.g.
        // "create a new shop").
        dashed:
