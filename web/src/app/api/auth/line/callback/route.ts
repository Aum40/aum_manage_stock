...
export async function GET(request: NextRequest) {
   ...
   * ดู app/api/users/link-line/start/route.ts)
   */
  ...
  if (isLinkFlow) {
    if (!code) {
      return done('/profile?connection=line&status=missing_code');
    }

    const result = await forwardAuthed('/users/me/link-line', {
      method: 'POST',
      body: { code },
    });
    const linked = result.status >= 200 && result.status < 300;
    return done(
      `/profile?connection=line&status=${linked ? 'linked' : 'failed'}`,
    );
  }
  ...
  if (!res.ok) {
    return done('/login?error=line_login_failed');
  }
